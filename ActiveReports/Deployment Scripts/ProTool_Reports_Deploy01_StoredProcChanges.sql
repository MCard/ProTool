...
* [ProTool].[ReportProjectExtract] - Removed columns and logic for specific, retired cost categories.
* [ProTool].[V_ProgramAnnualEstimates] - Removed retired cost category columns no longer used.
* [ProTool].[V_AnnualEstimates] - Removed retired cost category columns no longer used.
...
* [ProTool].[GetProjectExtract]  Old Get Project Extract still in use by the PowerPlant application.
***************************************************************************/
USE [ProTool_Staging]
GO
SET ANSI_NULLS ON
...
/*----------------------------------------------------------------------------------------------------
						VIEWS
------------------------------------------------------------------------------------------------------*/
/****** Object:  View [ProTool].[V_ProgramAnnualEstimates]    Script Date: 04/10/2013 16:51:11 ******/
Print 'Dropping View [ProTool].[V_ProgramAnnualEstimates]'
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[ProTool].[V_ProgramAnnualEstimates]'))
DROP VIEW [ProTool].[V_ProgramAnnualEstimates]
GO
/****** Object:  View [ProTool].[V_AnnualEstimates]    Script Date: 04/10/2013 16:51:11 ******/
Print 'Dropping View [ProTool].[V_AnnualEstimates]'
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[ProTool].[V_AnnualEstimates]'))
DROP VIEW [ProTool].[V_AnnualEstimates]
GO
/****** Object:  View [ProTool].[V_AnnualEstimates]    Script Date: 04/10/2013 16:51:11 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
Print 'Creating View [ProTool].[V_AnnualEstimates]'
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[ProTool].[V_AnnualEstimates]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW [ProTool].[V_AnnualEstimates]
AS
...
'
GO
IF NOT EXISTS (SELECT * FROM ::fn_listextendedproperty(N'MS_DiagramPane1' , N'SCHEMA',N'ProTool', N'VIEW',N'V_AnnualEstimates', NULL,NULL))
...
Begin DesignProperties = 
   ...
   End
End
' , @level0type=N'SCHEMA',@level0name=N'ProTool', @level1type=N'VIEW',@level1name=N'V_AnnualEstimates'
GO
IF NOT EXISTS (SELECT * FROM ::fn_listextendedproperty(N'MS_DiagramPaneCount' , N'SCHEMA',N'ProTool', N'VIEW',N'V_AnnualEstimates', NULL,NULL))
EXEC sys.sp_addextendedproperty @name=N'MS_DiagramPaneCount', @value=1 , @level0type=N'SCHEMA',@level0name=N'ProTool', @level1type=N'VIEW',@level1name=N'V_AnnualEstimates'
GO
...
GO
SET QUOTED_IDENTIFIER ON
Print 'Creating View [ProTool].[V_ProgramAnnualEstimates]'
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[ProTool].[V_ProgramAnnualEstimates]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW [ProTool].[V_ProgramAnnualEstimates]
AS
SELECT     a.Company_Cd, a.Project_No, a.Estimate_Year, a.Version_No, a.Gross - IsNull
                          ((SELECT     SUM(x.Gross)
                              FROM         ProTool.V_AnnualEstimates x INNER JOIN
...
FROM         ProTool.V_AnnualEstimates a INNER JOIN
                      ProTool.Projects b ON a.Company_Cd = b.Company_Cd AND a.Project_No = b.Project_No LEFT JOIN
                          (SELECT     d .Funding_Basket_Type_Cd, c.Estimate_Year, IsNull(SUM(c.Gross), 0) AS Gross, IsNull(Sum(c.Net), 0) AS Net, IsNull(Sum(c.Billings), 0) AS Billings, 
                            ...
                            FROM          ProTool.V_AnnualEstimates c INNER JOIN
...
FROM         ProTool.V_AnnualEstimates g INNER JOIN
...
'
GO
IF NOT EXISTS (SELECT * FROM ::fn_listextendedproperty(N'MS_DiagramPane1' , N'SCHEMA',N'ProTool', N'VIEW',N'V_ProgramAnnualEstimates', NULL,NULL))
EXEC sys.sp_addextendedproperty @name=N'MS_DiagramPane1', @value=N'[0E232FF0-B466-11cf-A24F-00AA00A3EFFF, 1.00]
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
   ...
   End
End
' , @level0type=N'SCHEMA',@level0name=N'ProTool', @level1type=N'VIEW',@level1name=N'V_ProgramAnnualEstimates'
GO
IF NOT EXISTS (SELECT * FROM ::fn_listextendedproperty(N'MS_DiagramPaneCount' , N'SCHEMA',N'ProTool', N'VIEW',N'V_ProgramAnnualEstimates', NULL,NULL))
EXEC sys.sp_addextendedproperty @name=N'MS_DiagramPaneCount', @value=1 , @level0type=N'SCHEMA',@level0name=N'ProTool', @level1type=N'VIEW',@level1name=N'V_ProgramAnnualEstimates'
GO
...
/****** Object:  StoredProcedure [ProTool].[ReportProjectExtract]    Script Date: 04/09/2013 09:43:27 ******/
Print 'Drop and Create Stored Procedure [ProTool].[ReportProjectExtract]'
if exists(select * from dbo.sysobjects where id = OBJECT_ID(N'[ProTool].[ReportProjectExtract]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [ProTool].[ReportProjectExtract]
GO
/* =================================================
...
--
-- UNIT TESTS
-- EXEC [ProTool].[ReportProjectExtract] '2013', 'TCO', 1, ' AND p.Project_No IN (17024) '
-- EXEC [ProTool].[ReportProjectExtract] '2013', 'TCO', 2, NULL
-- ================================================= */
CREATE PROCEDURE [ProTool].[ReportProjectExtract] 
...
--SELECT (
	...
	+ ' inner join'
	+ ' (select distinct a1.project_no,a1.company_cd from ProTool.V_AnnualEstimates as a1 WHERE a1.Estimate_Year=' + @Estimate_Year +') '
	...
	+ ' FROM V_ProgramAnnualEstimates ae '
	...
	+ ' FROM V_ProgramAnnualEstimates aeTot '
...
-- return 2nd result set
SELECT * FROM CostCategories

END
GO

Print 'Granting Execute permissions on Stored Procedure [ProTool].[ReportProjectExtract]'
GRANT EXECUTE ON [ProTool].[ReportProjectExtract] TO [db_FieldCostTracker]
GO
IF (EXISTS(SELECT * FROM sys.database_principals WHERE name = N'developers' AND type = 'R'))
GRANT EXECUTE ON [ProTool].[ReportProjectExtract] TO [developers]
GO
...
-- 05/06/2013   Mike Card             Changed @year to use current estimate year, not lattest year.
--
-- UNIT TESTS
-- EXEC [ProTool].[ReportForecastTotalsByCategory] 'TCO', 17024
-- EXEC [ProTool].[ReportForecastTotalsByCategory] 'CGT', 11
-- ================================================= */
...
AS
 BEGIN
	...
	-- get current year and its version
	--SELECT @year = MAX([Estimate_Year]) FROM #tblYearVersion --do not use lattest year.
	...
	-- 2nd, get rest of cost categories even if no Forecast rows exist.
	INSERT INTO #tblResults 
		([Company_Cd] 
		,[Project_No] 
		,[Year]
	...
	SELECT	@companyCode
	  ...
	  LEFT JOIN #tblResults r
		ON c.Cost_Category_Cd = r.Cost_Category_Cd
	  WHERE r.Company_Cd IS NULL 
			AND c.Cost_Category_Type = @categoryType
			AND c.Capital_Active_Fl = 1
	  ORDER BY c.Cost_Category_Cd

	-- 3rd, get totals for all years by company, project, cost category
...
-- 05/20/2013   Mike Card             Bug fix with join to Projects table.
--
-- UNIT TESTS
-- EXEC [ProTool].[ReportProjectChangeRequest_Main] 'TCO', 16764, 1
-- EXEC [ProTool].[ReportProjectChangeRequest_Main] 'NMS', 10, 1
-- ================================================= */
...
AS
 BEGIN
	...
	SELECT sc.[Company_Cd]
      ...
      ,CASE WHEN sc.[Scope_Decision] = 1 THEN app.Name ELSE '' END as [Scope_Approval_Name]
      ,CASE WHEN sc.[Scope_Decision] = 1 THEN app.Position ELSE '' END as [Scope_Approval_Role]
      ,CASE WHEN sc.[Scope_Decision] = 1 THEN sc.[Scope_Decision_Date] ELSE NULL END as [Scope_Approval_Date]
      ...
      ,(SELECT SUM([Amount])
		 ...
		 WHERE (Company_Cd = sc.Company_Cd 
			...
			AND Scope_Version_No = sc.Scope_Version_No)) as Change_Estimate_Total
  	...
  	LEFT JOIN [ProTool].[Projects] p (NOLOCK)
  		ON p.Company_Cd = sc.Company_Cd AND p.Project_No = sc.Project_No
	...
	LEFT JOIN #tblAgreement a
  	...
  	LEFT JOIN [ProTool].[User_Directory] app (NOLOCK) 
		ON app.TCOID = sc.[Scope_Decision_User]
...
--
-- UNIT TESTS
-- EXEC [ProTool].[ReportProjectChangeRequest_CostImpact] 'TCO', 16764, 2
-- ================================================= */
...
AS
 BEGIN
	...
	SELECT @versionNo = MAX([Version_No])
	FROM [ProTool].[Estimates] (NOLOCK)
	WHERE Company_Cd = @companyCode 
		AND Project_No = @projectNo 
...
GO
/* =================================================
-- SUMMARY: Old Get Project Extract still in use by the PowerPlant application.
-- 
-- DATE			DEVELOPER			  CHANGE HISTORY
----------------------------------------------------
-- 05/13/2013   Mike Card, Centric    Changed certain columns to return hard coded value of 0.
--
-- UNIT TESTS
...
SELECT @Group_Name = @select1
SELECT @selectlisttotal = @selectlist

SELECT @Where = '((ProTool.ProjectRoles.Capital_Active_Fl=1 AND ' + CAST(@Project_Type_Cd as varchar) + '=1) OR (ProTool.ProjectRoles.OM_Active_Fl=1 AND ' + CAST(@Project_Type_Cd as varchar) + '=2))' 
EXEC ProTool.Crosstab
	'SELECT P1.Company_Cd, P1.Project_No From (SELECT ProjectPeople.*,Users.UserName FROM ProjectPeople INNER JOIN Users on ProjectPeople.User_ID=Users.UserID WHERE Users.UserID IS NOT NULL) P1 Inner Join ProjectRoles on P1.Project_Role_Cd=ProjectRoles.Project_Role_Cd GROUP BY P1.Company_Cd, P1.Project_No',
--	'SELECT ProjectPeople.Company_Cd, ProjectPeople.Project_No From ProjectPeople Inner Join ProjectRoles on ProjectPeople.Project_Role_Cd=ProjectRoles.Project_Role_Cd GROUP BY ProjectPeople.Company_Cd, ProjectPeople.Project_No',
	'Max(UserName)',	
--	'MAX(User_ID)',
	'Project_Role_Name',
	'ProjectRoles',
...
EXEC	('select p.Company_Cd as Company,'
	...
	+ ' IsNull(OrigEstVerTPA.[Original Estimate Version - Total Project Amount],0) as [Original Estimate Version - Total Project Amount],'
 	...
 	+ ' IsNull(MaxEstVerTPA.[Max Estimate Version - Total Project Amount],0) as [Max Estimate Version - Total Project Amount],'
	+ ' SelYearATS.Version_No as [ '+ @Estimate_Year + ' - Approved To Spend, Estimate Version Number],'
	+ ' 0 as [ '+ @Estimate_Year +' - Approved To Spend, Labor],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Approved To Spend, Materials],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Approved To Spend, Outside Services],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Approved To Spend, Miscellaneous],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Approved To Spend, Overheads],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Approved To Spend, Cost of Retire],'
	+ ' IsNull(SelYearATS.Approved_To_Spend_Gross,0) as [ ' + @Estimate_Year + '  - Approved To Spend, Gross Project Cost for Year],'
	+ ' IsNull(SelYearATS.Billings,0) as [ ' + @Estimate_Year + '  - Approved To Spend, Calculated Billings],'
	+ ' IsNull(SelYearATS.Approved_To_Spend_Net,0) as [ ' + @Estimate_Year + '  - Approved To Spend, Net Project Cost for Year],'
	+ ' IsNull(SelYearTotalGross.Approved_To_Spend_Gross,0) as [ ' + @Estimate_Year + '  - Approved to Spend, Total Gross Project Estimate All Years],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Original Program, Labor],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Original Program, Materials],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Original Program, Outside Services],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Original Program, Miscellaneous],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Original Program, Overheads],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Original Program, Cost of Retire],'
	+ ' IsNull(SelYearOP.Original_Program_Gross,0) as [ ' + @Estimate_Year + '  - Original Program, Gross Project Cost for Year],'
	+ ' IsNull(SelYearOP.Billings,0) as [ ' + @Estimate_Year + '  - Original Program, Calculated Billings],'
	+ ' IsNull(SelYearOP.Original_Program_Net,0) as [ ' + @Estimate_Year + '  - Original Program, Net Project Cost for Year],'
	+ ' IsNull(SelYearTotalGross.Original_Program_Gross,0) as [ ' + @Estimate_Year + '  - Original Program, Total Gross Project Estimate All Years],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Preliminary Program, Labor],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Preliminary Program, Materials],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Preliminary Program,Outside Services],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Preliminary Program, Miscellaneous],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Preliminary Program, Overheads],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Preliminary Program, Cost of Retire],'
	+ ' IsNull(SelYearPP.Preliminary_Program_Gross,0) as [ ' + @Estimate_Year + '  - Preliminary Program, Gross Project Cost for Year],'
	+ ' IsNull(SelYearPP.Billings,0) as [ ' + @Estimate_Year + '  - Preliminary Program, Calculated Billings],'
	+ ' IsNull(SelYearPP.Preliminary_Program_Net,0) as [ ' + @Estimate_Year + '  - Preliminary Program, Net Project Cost for Year],'
	+ ' IsNull(SelYearTotalGross.Preliminary_Program_Gross,0) as [ ' + @Estimate_Year + '  - Preliminary Program, Total Gross Project Estimate All Years],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Present Estimate, Labor],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Present Estimate, Materials],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Present Estimate,Outside Services],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Present Estimate, Miscellaneous],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Present Estimate, Overheads],'
	+ ' 0 as [ ' + @Estimate_Year + '  - Present Estimate, Cost of Retire],'
	+ ' IsNull(SelYearPE.Gross,0) as [ ' + @Estimate_Year + '  - Present Estimate, Gross Project Cost for Year],'
	+ ' IsNull(SelYearPE.Billings,0) as [ ' + @Estimate_Year + '  - Present Estimate, Calculated Billings],'
	+ ' IsNull(SelYearPE.Net,0) as [ ' + @Estimate_Year + '  - Present Estimate, Net Project Cost for Year],'
	--+ ' IsNull(SelYearPETotalGross.Gross,0) as [ ' + @Estimate_Year + '  - Present Estimate, Total Gross Project Estimate All Years],'
	+ ' 0 as [YTD Actuals, Labor],'
	+ ' 0 as [YTD Actuals, Materials],'
	+ ' 0 as [YTD Actuals, Outside Services],'
	+ ' 0 as [YTD Actuals, Miscellaneous],'
	+ ' 0 as [YTD Actuals, Overheads],'
	+ ' 0 as [YTD Actuals, Cost of Retire],'
	+ ' IsNull(CAST(YTDActuals.Billings as decimal(20,2)),0) as [YTD Actuals, Billings/Reimbursement],'
	+ ' IsNull(CAST(YTDActuals.Accruals as decimal(20,2)),0) as [YTD Actuals, Accruals],'
	+ ' IsNull(CAST(YTDActualsGross.Gross as decimal(20,2)),0) as [YTD Actuals, Gross Project Cost],'
	+ ' IsNull(CAST(YTDActualsNet.Net as decimal(20,2)),0) as [YTD Actuals, Net Project Cost],'
	+ ' 0 as [PTD Actuals, Labor],'
	+ ' 0 as [PTD Actuals, Materials],'
	+ ' 0 as [PTD Actuals, Outside Services],'
	+ ' 0 as [PTD Actuals, Miscellaneous],'
	+ ' 0 as [PTD Actuals, Overheads],'
	+ ' 0 as [PTD Actuals, Cost of Retire],'
	+ ' IsNull(CAST(PTDActuals.Billings as decimal(20,2)),0) as [PTD Actuals, Billings/Reimbursement],'
	+ ' IsNull(CAST(PTDActuals.Accruals as decimal(20,2)),0) as [PTD Actuals, Accruals],'
	+ ' IsNull(CAST(PTDActualsGross.Gross as decimal(20,2)),0) as [PTD Actuals, Gross Project Cost],'
	+ ' IsNull(CAST(PTDActualsNet.Net as decimal(20,2)),0) as [PTD Actuals, Net Project Cost]'
	+ ' from ProTool.Projects as p'
	+ ' inner join'
...
-- 	+ ' SELECT aeTot.Project_No'
-- 	+ ' ,aeTot.Company_Cd'
-- 	+ ' ,SUM(aeTot.Original_Program_Gross) as Gross'
-- 	+ ' FROM ProgramAnnualEstimates aeTot '
-- 	+ ' WHERE aeTot.Original_Program_Fl=1'
-- 	+ ' GROUP BY aeTot.Project_No, aeTot.Company_Cd'
-- 	+ ' ) as SelYearOPTotalGross'
-- 	+ ' on	'
-- 	+ ' p.Company_Cd = SelYearOPTotalGross.Company_Cd AND p.Project_No = SelYearOPTotalGross.Project_No'
	+ ' left join'
	+ ' ('
	+ ' SELECT ae.Project_No'
	+ ' ,ae.Company_Cd'
...
-- 	+ ' SELECT aeTot.Project_No'
-- 	+ ' ,aeTot.Company_Cd'
-- 	+ ' ,SUM(aeTot.Preliminary_Program_Gross) as Gross'
-- 	+ ' FROM ProgramAnnualEstimates aeTot '
-- 	+ ' WHERE aeTot.Preliminary_Program_Fl=1'
-- 	+ ' GROUP BY aeTot.Project_No, aeTot.Company_Cd'
-- 	+ ' ) as SelYearPPTotalGross'
-- 	+ ' on	'
-- 	+ ' p.Company_Cd = SelYearPPTotalGross.Company_Cd AND p.Project_No = SelYearPPTotalGross.Project_No'
	+ ' left join'
	+ ' 	('
	+ ' SELECT ae.Project_No'
	+ ' ,ae.Company_Cd'
...
-- 	+ ' SELECT aeTot.Project_No'
-- 	+ ' ,aeTot.Company_Cd'
-- 	+ ' ,SUM(aeTot.Gross) as Gross'
-- 	+ ' FROM ProgramAnnualEstimates aeTot '
-- 	+ ' WHERE aeTot.Version_No in (Select max(Version_No) '
-- 	+ ' from Protool.Estimates est '
-- 	+ ' where est.Project_No = aeTot.Project_No' 
-- 	+ ' and est.Company_CD = aeTot.Company_CD)'
-- 	+ ' GROUP BY aeTot.Project_No, aeTot.Company_Cd'
-- 	+ ' ) as SelYearPETotalGross'
-- 	+ ' on	'
-- 	+ ' p.Company_Cd = SelYearPETotalGross.Company_Cd AND p.Project_No = SelYearPETotalGross.Project_No'
	+ ' left join'
	+ ' ('
	+ ' SELECT '	
	+ ' SumExp.Company_Cd, '
	+ ' SumExp.Project_No, '
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 1 THEN SumExp.Amount ELSE 0 END) as Labor,'
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 2 THEN SumExp.Amount ELSE 0 END) as Materials,'
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 3 THEN SumExp.Amount ELSE 0 END) as [Outside Services],'
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 4 THEN SumExp.Amount ELSE 0 END) as Miscellaneous,'
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 5 THEN SumExp.Amount ELSE 0 END) as Overheads,'
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 6 THEN SumExp.Amount ELSE 0 END) as [Cost of Retire],'
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 7 THEN SumExp.Amount ELSE 0 END) as [Billings],'
	+ ' SUM(CASE SumExp.Exp_Cost_Category_Cd'
	+ ' WHEN 8 THEN SumExp.Amount ELSE 0 END) as Accruals,'
	...
	+ ' WHERE'
	+ @YTD_Flag
	--+ ' SumExp.Exp_Voucher_Year='+@Estimate_Year+' AND SumExp.Exp_Voucher_Month <= MONTH(getDate())'
	+ ' GROUP BY'
	+ ' SumExp.Company_Cd, SumExp.Project_No'
	+ ' ) YTDActuals'
	+ ' on'
	...
	+ ' p.Company_Cd = PTDActualsNet.Company_Cd AND p.Project_No = PTDActualsNet.Project_No '
--	+ ' left join ('
--	+ ' SELECT    aud.Company_Cd, aud.Project_No, aud.Audit_Date '
--	+ ' FROM         Audits aud'
--	+ ' WHERE     (aud.Audit_Event_ID = 44) AND (aud.Audit_Date IN'
--	+ '                           (SELECT     MAX(aud1.audit_date)'
--	+ '                             FROM          Audits aud1'
--	+ '                             WHERE      aud.project_no = aud1.project_no AND aud.company_cd = aud1.company_cd))'
--	+ ' ) as ProjRev'
--	+ ' on p.Company_cd=ProjRev.Company_cd and p.Project_No=ProjRev.Project_No'
	+ ' left join ('
	+ ' SELECT    aud.Company_Cd, aud.Project_No, max(aud.Audit_Date) as Audit_Date '
	...
	+ ' ON p.Company_Cd=pr.Company_Cd AND p.Project_No=pr.Project_No'
	+ @where
--	+ ' AND p.Project_No IN (' + @Project_List + ')'
	+ @Project_List + @Project_List_2 + @Project_List_3 + @Project_List_4 + @Project_List_5 + @Project_List_6 + @Project_List_7
	+ ' AND Users.UserID IS NOT NULL'
	+ ' order by '
	+ ' p.Company_Cd,p.Project_No')
